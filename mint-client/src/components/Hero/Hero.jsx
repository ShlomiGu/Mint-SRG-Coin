import React from "react";
import { styled, Grid, Button, Typography } from "@mui/material";
import Carousel from "./Carousel";

const HeroContainer = styled("div")({
  width: "86vw",
  minHeight: "90vh",
  display: "flex",
  flexDirection: "column",
  overflow: "hidden",
  padding: "5vh 7vw",
});

const BuyButton = styled(Button)({
  width: "280px",
  height: "80px",
  background: "#FFCC00 0% 0% no-repeat padding-box",
  border: "2px solid #FFCC00",
  borderRadius: "8px",
  boxShadow: "2px 2px 20px #FFCC00",
  fontSize: "1.5rem",
  marginTop: "20px",
  fontWeight: "bold",
  textTransform: "none",
  color: "black",
  "&:hover": {
    color: "#FFCC00",
  },
});

const TitleText = styled(Typography)({
  color: "white",
  fontSize: "2.75em",
  fontWeight: "bold",
  "@media (max-width:768px)": {
    fontSize: '1.75em'
  },
});

const GoldBarImgae = styled("img")({
  height: "2em",
  marginLeft: "20px",
  verticalAlign: "middle",
});

const LinesBg = styled("img")({
  position: "absolute",
  top: 0,
  right: 0,
  width: "inherit",
});

const LogoIcon = styled('img')({
  width: '12.5%',
  '@media (max-width:768px)':{
    width: '40%'
  }
})

const HeroSection = () => {
  return (
    <HeroContainer
      style={{
        backgroundImage: 'url("assets/DesktopSkyBackground.png")',
        backgroundSize: "cover",
        backgroundRepeat: "no-repeat",
      }}
    >
      <LinesBg src="assets/Main.png" />
      <Grid item container xs={12} zIndex={10}>
        <Grid item xs={8}>
          <LogoIcon src={"assets/logo.png"} alt="logo"/>
        </Grid>
        <Grid item xs={4}>
          {/* <img className="Cube-1" src={"/assets/Cube1.png"} alt="cube1"/> */}
          <img className="Cube-1" src={"/assets/Cube1.png"} alt="cube1" width={'40%'}/>
        </Grid>
      </Grid>
      <Grid item container xs={12} zIndex={10}>
        <Grid item xs={10}>
          <TitleText zIndex={10}>
            <span>
              From the dawn of history mankind has given value to coins through
              a backup reserve. The oldest and most reliable being
            </span>
            <GoldBarImgae src="assets/goldbar.png" alt="goldbar" />
          </TitleText>
        </Grid>
        <Grid item xs={2}>
          <img className="Cube-2" src={"/assets/Cube2.png"} alt="cube2" width={'40%'}/>
        </Grid>
      </Grid>
      <Grid item container zIndex={10}>
        <Grid item>
          <BuyButton>{"Transfer/Buy"}</BuyButton>
        </Grid>
        <Grid item>
          <Carousel buttons/>
        </Grid>
      </Grid>
    </HeroContainer>
  );
};

export default HeroSection;
